...
import sys
import os # 確保 os 在頂層可用，用於環境變數

# 使用頂層 try-except 塊來捕獲任何在應用程式啟動時發生的未處理異常
try:
    ...
    # 設定 Gemini API 金鑰
    # IMPORTANT: 在生產環境中，請務必從環境變數中讀取 API 金鑰，不要硬編碼！
    # 例如：在 Railway 或其他部署平台設定名為 GEMINI_API_KEY 的環境變數。
    GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "") 

    # === 新增的除錯訊息：打印 GEMINI_API_KEY 的值 (使用 !r 顯示原始字串) ===
    # 這裡打印的是從環境變數中讀取到的 API Key。
    ...
    def extract_text_from_pdf(file_stream):
        ...
        try:
            ...
            print(f"PDF 解析錯誤: {e}", file=sys.stderr)
    ...
    def extract_content_from_docx(file_stream):
        ...
        try:
            ...
            print(f"DOCX 解析錯誤: {e}", file=sys.stderr)
    ...
    def call_gemini_api(cv_content):
        ...
        if not GEMINI_API_KEY:
            print("錯誤: GEMINI_API_KEY 環境變數未設定或為空！請確認您已在 Railway 環境中設定 GEMINI_API_KEY。", file=sys.stderr)
        ...
        instruction_text = """
            ...
            2. 原因 (reason): 為什麼這是一個疑點，請簡潔說明，**不超過50個中文字元**，例如「無公開紀錄」、「資料不符」、「資訊不完整」。
        ...
        payload = {
            ...
            "generationConfig": {
                ...
                "maxOutputTokens": 2048, # 限制模型輸出長度，避免截斷
                "responseSchema": {
                    "type": "OBJECT",
                    "properties": {
        ...
        for attempt in range(max_retries):
            try:
                ...
                # === 除錯打印：打印最終的 URL 字串 (API Key 來自環境變數) ===
                ...
                print(f"警告: 呼叫 Gemini API 超時 (第 {attempt + 1} 次嘗試): {e}", file=sys.stderr)
                if attempt < max_retries - 1:
                    sleep_time = 2 ** attempt # 指數退避策略
                    print(f"等待 {sleep_time} 秒後重試...", file=sys.stderr)
                ...
                print(f"錯誤: 解析 Gemini API 回應失敗 (第 {attempt + 1} 次嘗試): {e}", file=sys.stderr)
    ...
    def upload_cv():
        print("DEBUG: /upload_cv endpoint hit.")
        if 'cvFile' not in request.files:
            print("ERROR: No file part in the request.", file=sys.stderr)
            return jsonify({"error": "沒有檔案被上傳"}), 400

        file = request.files['cvFile']
        if file.filename == '':
            print("ERROR: No selected file.", file=sys.stderr)
        ...
        try:
            ...
            if not cv_content.strip():
                print("ERROR: File content is empty or could not extract valid text.", file=sys.stderr)
                return jsonify({"error": "檔案內容為空或無法提取有效文本"}), 400

            print("DEBUG: CV content extracted. Calling Gemini API...")
            ...
            if isinstance(llm_response, dict) and "candidates" in llm_response and \
               ...
               len(llm_response["candidates"][0]["content"]["parts"]) > 0:
                ...
                try:
                    ...
                    print(f"ERROR: Failed to parse LLM response JSON string: {e}", file=sys.stderr)
                    print(f"ERROR: String causing error (first 500 chars): {cleaned_response_text[:500]!r}...", file=sys.stderr)
                    print(f"ERROR: Length of string causing error: {len(cleaned_response_text)}", file=sys.stderr)
            ...
            if "hasDiscrepancies" in parsed_llm_json and isinstance(parsed_llm_json["hasDiscrepancies"], bool) and \
               "discrepancies" in parsed_llm_json and isinstance(parsed_llm_json["discrepancies"], list):
                print("DEBUG: LLM response parsed and validated successfully.")
                return jsonify(parsed_llm_json), 200
            else:
                print("ERROR: LLM response JSON structure does not match expected schema.", file=sys.stderr)
                return jsonify({"error": "LLM 回應的 JSON 結構不符預期"}), 500
        else:
            print("ERROR: LLM response format is invalid or empty.", file=sys.stderr)
            return jsonify({"error": "LLM 回應格式無效或為空"}), 500

        except ValueError as e:
            print(f"ERROR: Client-side error: {e}", file=sys.stderr)
            return jsonify({"error": str(e)}), 400
        except ConnectionError as e:
            print(f"ERROR: Connection error to external service: {e}", file=sys.stderr)
            return jsonify({"error": str(e)}), 503 # Service Unavailable
        except Exception as e:
            print(f"CRITICAL ERROR: Internal server error: {e}", file=sys.stderr)
            return jsonify({"error": f"伺服器內部錯誤: {e}"}), 500

    # Railway 會將請求發送到 $PORT 環境變數指定的埠號
    if __name__ == '__main__':
        port = int(os.environ.get("PORT", 8080))
        print(f"DEBUG: Running app.py directly on port {port} (This block is for local testing, Gunicorn handles production).")
        app.run(host='0.0.0.0', port=port, debug=True)

except Exception as e:
    # 捕獲並打印任何在模組載入或頂層執行時發生的異常
    print(f"CRITICAL ERROR: Application failed to start due to an unhandled exception during initialization: {e}", file=sys.stderr)
    import traceback
    traceback.print_exc(file=sys.stderr) # 打印完整的 traceback
    sys.exit(1) # 以非零狀態碼退出，表示啟動失敗